...
class H5Dataset(Dataset):
    ...
    def join_structures(structures):
        ...
        if structure is None or callable(structure) or isinstance(structure, (FunctionWithSource, np.ndarray, int, bytes, float, bool, PIL.Image.Image, str,type(type(None)))):
            ...
            return [H5Dataset.join_structures({key_structure:(structures[key_structure][index] if (structures[key_structure] is not None and index<len(structures[key_structure])) else None) for key_structure in structures}) for index in range(len(structure))]
        elif isinstance(structure, dict):
            return {key: H5Dataset.join_structures({key_structure: (structures[key_structure][key] if (structures[key_structure] is not None and key in structures[key_structure]) else None) for key_structure in structures}) for key in structure}
        else:
